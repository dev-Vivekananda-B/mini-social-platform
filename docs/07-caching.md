...
* **User Feed Lists:** A list of `Post_IDs` tailored to a specific user (e.g., `user_123_feed: [post_89, post_90, post_102]`). 
* **Post Metadata:** The actual text and metadata of a post (Author ID, Caption, Timestamp, Like Count, Media URLs). Cached as a simple Key-Value pair (e.g., `post_89: { ... }`).
...
* **How:** We use a distributed, memory-resident datastore like **Redis** or **Memcached**. 
  * **Redis** is preferred for Feed Lists because its native `List` and `Sorted Set` data structures make it trivial to append new `Post_IDs` or paginate through a feed chronologically.
...
* **Standard Users (Push / Fan-out on Write):** If a user has 500 followers, we synchronously append the new `Post_ID` to the cached Feed Lists of all 500 followers.
...
* We use a **Cache-Aside** pattern. When a post is deleted or the caption is edited, the application updates the database and immediately issues an `EXPIRE` or `DEL` command to the specific `post_{id}` key in Redis. 
